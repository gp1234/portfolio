...
class ThreeSphere {
  ...
  private readonly pointCount = 10000;
  ...
  init(): void {
    ...
    console.log("ThreeSphere: Setting up OrbitControls...");
    try {
      ...
      console.log("ThreeSphere: OrbitControls configured successfully");
      ...
      console.error("ThreeSphere: Failed to initialize OrbitControls:", error);
  ...
  private animate = (): void => {
    ...
    const { points, renderer, scene, camera, controls } = this.manager;

    // Update controls if available
    if (controls && typeof controls.update === "function") {
      controls.update();
    } else if (points) {
      // Fallback to manual rotation if no controls
      points.rotation.y += 0.002;
      points.rotation.x += 0.001;
    }

    // Update point sizes for animations
    if (points) {
  ...
  destroy(): void {
    ...
    if (
      ...
      typeof this.manager.controls.dispose === "function"
    ) {
      this.manager.controls.dispose();
    }
...
export function initSphereAnimation() {
  console.log("initSphereAnimation called");
  ...
  if (body && body.style.display !== "none") {
    ...
    setTimeout(() => initSphereAnimation(), 50);
  }
}
